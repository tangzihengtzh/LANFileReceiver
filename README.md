...
- 应用不负责开关热点，需要用户手动开启
- 传输记录仅保留在内存中，进程结束即清空（后续可迁移到 Room）
- 暂未实现断点续传与二维码
...
- Transfer history lives in memory only and is cleared when the process dies (Room migration planned)
- No resume-after-interruption and no QR code yet
